    ...
    <div class="row">
        @foreach (LaravelLocalization::getSupportedLocales() as $language => $value)
            <div class="col-md-6 col-12">
                <div class="form-group">
                    <label for="title">{{ trans('main_translation.OfferTitle_' . $language) }}</label>
                    <div class="controls">
                        <input type="text" name="title[{{ $language }}]" id="title"
                            class="form-control @error("title.$language") is-invalid @enderror"
                            placeholder="{{ trans('main_translation.EnterOfferTitle_' . $language) }}"
        ...
        <div class="col-lg-6 col-md-12">
            <fieldset class="form-group">
                <label for="offer_image">{{ trans('main_translation.UploadImage') }}</label>
            ...
            <div class="form-group">
                ...
                <fieldset id="status" class="form-group">
                    <select name="status" class="form-control @error('status') is-invalid @enderror" id="basicSelect">
                        <option value="show" @if ($offer->status == 'show' or old('status') == 'show') selected @endif>
                            {{ trans('main_translation.Show') }}
                        </option>
                        <option value="hide" @if ($offer->status == 'hide' or old('status') == 'hide') selected @endif>
